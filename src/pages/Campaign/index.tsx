...
export default function Campaign() {
    return (
        <section className="container max-w-5xl px-2 mx-auto pt-3 h-full flex flex-col gap-2 justify-start items-center">
            ...
            <DynamicTable defaultAction={(e: any) => alert(e.id)} columns={Columns}
                pagination={
                    { cant: 10, totalPages: 100, first: () => alert('first'), last: () => alert('last'), page: 0, next:()=>alert('next'),prev:()=>alert('prev') }
                }
                rows={
                    [
